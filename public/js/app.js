/* LearnHub — E-Learning & Community Platform (v1.1)
   Features: Courses, Enrollments, Quizzes, Attempts, Certificates, Chat, Tasks, Profiles
   Roles: student | instructor | admin  (stored in roles/{uid}.role) + email override
*/
(() => {
  'use strict';
  ...
  const stg  = firebase.storage(); // ensure storage compat is loaded in index.html
      ...
      <div class="card"><div class="card-body">
        ...
        <div class="grid cols-2">
          <div><label>Palette</label>
            <select id="theme-palette" class="input"><option value="sunrise">sunrise</option><option value="dark">dark</option></select>
          </div>
          <div><label>Font size</label>
            <select id="theme-font" class="input"><option value="small">small</option><option value="medium" selected>medium</option><option value="large">large</option></select>
          </div>
        </div>
  ...
  function render(){
    ...
    wireShell(); wireRoute();
    // scroll focus after search open
  ...
  function wireProfile(){
    ...
    $('#main').addEventListener('click', async (e)=>{
      ...
      ctx.fillStyle='#0b0d10'; ctx.fillRect(0,0,1200,800);
      ctx.fillStyle='#fff'; ctx.font='bold 48px Inter'; ctx.fillText('Certificate of Completion', 280, 200);
      ctx.font='28px Inter'; ctx.fillText(`Awarded to: ${p.name||p.email}`, 280, 280);
      ctx.fillText(`Course: ${course.title||courseId}`, 280, 330);
      ctx.fillText(`Date: ${new Date().toLocaleDateString()}`, 280, 380);
      ctx.strokeStyle='#7ad3ff'; ctx.lineWidth=6; ctx.strokeRect(80,120,1040,560);
  ...
  // Dev helper: seed sample data
  window.seedSampleData = async function(){
    const u=auth.currentUser; if(!u) return alert('Sign in first');
    // 2 courses
    const c1=await col('courses').add({title:'Algebra Basics',category:'Math',desc:'Equations, functions, factoring.',ownerUid:u.uid,ownerEmail:u.email,createdAt:firebase.firestore.FieldValue.serverTimestamp()});
    ...
    // quiz
    await col('quizzes').add({title:'Algebra Quiz 1',courseId:c1.id,courseTitle:'Algebra Basics',passScore:70,items:[{q:'2+2?',choices:['3','4','5'],answer:1},{q:'5x=20, x=?',choices:['2','4','5'],answer:2}],ownerUid:u.uid,createdAt:firebase.firestore.FieldValue.serverTimestamp()});
