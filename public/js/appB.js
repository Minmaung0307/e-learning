...
await db.collection('roles').doc(uid).set({ uid, role }, { merge: true });</code></pre>
  ...
  const obj = {
    ...
    ownerUid: auth.currentUser.uid,                 // REQUIRED by rules
...
if(tg){
  ...
  if(block){
    const isClamped = block.classList.toggle('clamp');
    tg.textContent = isClamped ? 'Read more' : 'Read less';
  }
}
...
const tgl = e.target.closest?.('[data-short-toggle]');
if(tgl){
  const wrap = tgl.closest('.card-body') || tgl.parentElement;
  const block = wrap.querySelector('.short');
  if(block){
    const isClamped = block.classList.toggle('clamp');
    tgl.textContent = isClamped ? 'Read more' : 'Read less';
  }
}
